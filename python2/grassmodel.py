...
def encode_structure_fold(fold, tree):
    ...
    def encode_node(node):
        if node.is_leaf():
            ...
            return fold.add(u'adjNode', left, right)
        elif node.is_sym():
            feature = encode_node(node.left)
            sym = node.sym
            return fold.add(u'symNode', feature, sym)
...
class GRASSDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.creLoss = nn.CrossEntropyLoss()  # pytorch's negative log likelihood loss
    ...
    def boxLossEstimator(self, box_feature, gt_box_feature):
        return torch.cat([self.mseLoss(b, gt).mul(0.4) for b, gt in izip(box_feature, gt_box_feature)], 0)

    def symLossEstimator(self, sym_param, gt_sym_param):
        return torch.cat([self.mseLoss(s, gt).mul(0.5) for s, gt in izip(sym_param, gt_sym_param)], 0)

    def classifyLossEstimator(self, label_vector, gt_label_vector):
        return torch.cat([self.creLoss(l.unsqueeze(0), gt).mul(0.2) for l, gt in izip(label_vector, gt_label_vector)], 0)
...
def decode_structure_fold(fold, feature, tree):
    ...
    def decode_node_box(node, feature):
        if node.is_leaf():
            ...
            left, right = fold.add(u'adjDecoder', feature).split(2)
            ...
            sym_gen, sym_param = fold.add(u'symDecoder', feature).split(2)
            sym_param_loss = fold.add(u'symLossEstimator', sym_param, node.sym)
            sym_gen_loss = decode_node_box(node.left, sym_gen)
            label = fold.add(u'nodeClassifier', feature)
            label_loss = fold.add(u'classifyLossEstimator', label, node.label)
            loss = fold.add(u'vectorAdder', sym_gen_loss, sym_param_loss)
            return fold.add(u'vectorAdder', loss, label_loss)
